...
module.exports = function initialize(agent, library, spec) {
  ...
  function wrappedPromise(executor) {
    logger.trace(
      ...
      'Promise instrumentation: wrappedPromise'
    )
  ...
  function wrapResolver(context, fn, name) {
    logger.trace(
      { resolver: name, stackTrace: new Error().stack },
      'Promise instrumentation: wrapResolver'
    )
  ...
  function _wrapThen(then, name, useAllParams) {
    logger.trace(
      { stackTrace: new Error().stack },
      'Promise instrumentation: _wrapThen'
    )
  ...
  function wrapPromisifiy(promisify, name) {
    ...
    return function __NR_wrappedPromisify() {
      ...
      logger.trace('Promise instrumentation: __NR_wrappedPromisify end')
      return function __NR_wrappedPromisified() {
        logger.trace('Promise instrumentation: __NR_wrappedPromisified')
